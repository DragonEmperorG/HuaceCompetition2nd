...
def make_dataset():
    ...
    # 要生成的文件
    dataset_writer = tf.python_io.TFRecordWriter(os.path.join(cwd, 'dataset', "ion_dataset_ms.tfrecords"))
    training_writer = tf.python_io.TFRecordWriter(os.path.join(cwd, 'dataset', "ion_training_ms.tfrecords"))
    validation_writer = tf.python_io.TFRecordWriter(os.path.join(cwd, 'dataset', "ion_validation_ms.tfrecords"))
    test_writer = tf.python_io.TFRecordWriter(os.path.join(cwd, 'dataset', "ion_test_ms.tfrecords"))
    ...
    for img_name in tqdm(os.listdir(raw_data_path), desc="TFRecordWriterProgress", unit="mat", ascii=True):
        ...
        if (time_step_counter == 0):
            iot_img_sequences = img_raw_array
        else:
            iot_img_sequences = np.concatenate((iot_img_sequences, img_raw_array), axis=-1)
        ...
        if (time_step_counter >= iot_time_steps):
            ...
            iot_img_sequences = iot_img_sequences[:,:,1:]
